...
class _CalculatorState extends State<Calculator> {
      ...
      return ExpansionTile(
        ...
        title: Padding(
          ...
          child: Column(
            children: [
              ...
              //   title: "납입(지연/선납)",
              //   value:
              //       "${stats['totalNormalCount']}회(${stats['totalDelayDayCount']}회/${stats['totalPrepaidDayCount']}회)",
              // ),
              Row(
                ...
                children: [
                  ...
                  Text(
                    "정상:${stats['totalNormalCount']}회 지연:${stats['totalDelayDayCount']}회 선납:${stats['totalPrepaidDayCount']}회",
        ...
        children: payments.map((payment) {
          return ListTile(
            ...
            //   children: [
            //     Text("${payment.installmentNo} 회차"),
            //     const SizedBox(width: 8),
            //     if (payment.delayDays != null && payment.delayDays > 0)
            //       Icon(
            //         Icons.fast_rewind,
            //         color: Theme.of(context).colorScheme.error,
            //       ),
            //     if (payment.prepaidDays != null && payment.prepaidDays > 0)
            //       Icon(
            //         Icons.fast_forward,
            //         color: Theme.of(context).colorScheme.tertiary,
            //       ),
            //     if ((payment.delayDays == 0 || payment.delayDays == null) &&
            //         (payment.prepaidDays == 0 || payment.prepaidDays == null))
            //       Icon(
            //         Icons.check,
  ...
  /// Builds the schedule summary card shown below the list
      ...
      child: Padding(
        ...
        child: Row(
          ...
          children: [
            Flexible(
              child: Text(
                "총 회차: ${schedule.totalInstallments} | 연체합: ${schedule.totalDelayDays}일 | 선납합: ${schedule.totalPrepaidDays}일",
  ...
  Widget _buildInstallmentNoIcon(String installmentNo, String status) {
    Color color;
    if (status == "delay") {
      color = Theme.of(context).colorScheme.error;
    ...
    return Container(
      ...
      height: 36,
      decoration: BoxDecoration(
        color: color.withValues(alpha: 0.15), // 은은한 배경색
        shape: BoxShape.rectangle,
        border: Border.all(color: color, width: 2), // 진한 보더라인
      ),
      alignment: Alignment.center,
      child: Text(
        "$installmentNo회차",
  ...
  Future<void> _onSave() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        final updatedPayment = updated.payments.firstWhere(
          (p) => p.installmentNo == currentPayment.installmentNo,
